...
import { Ship, Clock, Anchor, Package, ArrowLeft, Loader2, AlertCircle, CheckCircle2, Info, Lock } from 'lucide-react';
...
export default function PublicTrackingDetailPage({ params }: { params: { id: string } }) {
    ...
    const [error, setError] = useState<{ message: string; title: string; type: 'auth' | 'notfound' | 'error' } | null>(null);

    const voyageCode = searchParams.get('code');
        ...
        const fetchVoyage = async () => {
            if (!voyageCode || !phone) {
                setError({
                    ...
                    message: 'Thiếu thông tin tra cứu (Mã chuyến hoặc SĐT)',
                    type: 'auth'
                });
                setIsLoading(false);
                return;
        ...
        return (
            <div className="flex flex-col items-center justify-center p-6 md:p-12 min-h-[60vh]">
                <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-red-100 p-8 text-center animate-in fade-in zoom-in duration-300">
                    ...
                    >
                        <ArrowLeft size={14} strokeWidth={3} /> Quay lại tra cứu
                    </button>
                </div>
            </div>
        );
                                    ...
                                    <div key={log.id} className="relative pl-10 animate-in fade-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-1">
                                                <h4 className="text-sm font-black text-slate-900 uppercase tracking-tight font-heading">{log.title}</h4>
